...
export default function Library() {
    ...
    const handleDelete = async(book_id) => {
        const newRecDel = await LibraryService.delete(book_id).then(LibraryService.getAll().then(setLibrary))
    }

    const handleEdit = async(book_id) => {
        const newRecUpd = await LibraryService.update(book_id).then(LibraryService.getAll().then(setLibrary))
    }
                                    ...
                                    return(
                                        <div key={lib.book_id} className="col-md-6">
                                            <div className="card mb-3 position-relative">
                                                <div className="card-title text-primary text-center">{lib.title}</div>
                                                <hr />
                                                <div className="card-body">
                                                    ...
                                                    onClick={()=> {handleEdit(lib.book_id)}}>
                                                        Edit
                                                    </button>
                                                </div>    
                                            </div>
