...
const update = "update Books set title=?, publication_year=?, author=?, genre=?, availability=? "
...
app.put("/books/:id", (req, res) => {
    const{book_id} = req.params;
    const {title, publication_year, author, genre, availability} = req.body;
    db.query(update, [book_id, title, publication_year, author, genre, availability], (err,results) =>{
        if(err) return res.status(500).json({error: err.message});
